...
export function TireSizeSection({ width, aspectRatio, diameter, construction, onChange }: TireSizeProps) {
  return (
    <div className="space-y-4">
      <label className="block text-sm font-medium !text-black">Tire Size</label>
      
      {/* First row: Width, Aspect Ratio, Diameter */}
      <div className="grid grid-cols-3 gap-4">
        {/* Width Column */}
        <div>
          <label className="block text-xs !text-black mb-2">Width (mm)</label>
          <div className="grid grid-cols-3 gap-1 mb-2">
            {[175, 185, 195, 205, 215, 225, 235, 245, 255, 265].map((w) => (
              <button
                ...
                className="px-2 py-1.5 text-sm rounded border border-gray-300 hover:bg-gray-100 !text-black"
              >
                {w}
              </button>
            ))}
          </div>
          <div className="h-[38px]"> {/* Add fixed height container */}
            <input
              type="number"
              value={width || ''}
        ...
        {/* Aspect Ratio Column */}
        <div>
          <label className="block text-xs !text-black mb-2">Aspect Ratio</label>
          <div className="grid grid-cols-3 gap-1 mb-2">
            {[30, 35, 40, 45, 50, 55, 60, 65, 70, 75].map((ratio) => (
              <button
                key={ratio}
                type="button"
        ...
        {/* Diameter Column */}
        <div>
          <label className="block text-xs !text-black mb-2">Diameter (inches)</label>
          <div className="grid grid-cols-3 gap-1 mb-2">
      ...
      <div className="mt-4">
        <div className="w-full md:w-1/3">
          <label className="block text-xs !text-black mb-2">Construction</label>
          <select
            value={construction}
